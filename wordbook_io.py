...
크롤러는 np.array 첫 행에 라벨(["japanese", "Hanmoon", ...])을 넣고
pd.DataFrame(...).to_excel(index=False) 로 저장하므로, 엑셀 첫 줄은 0,1,2,... 이고
둘째 줄이 실제 라벨이다. 라벨 행을 찾아서 헤더로 사용한다.
"""
import re
...
def load_crawl(path):
    ...
    if "pos" not in df.columns:
        df["pos"] = ""
    df = df[["japanese", "hanmoon", "level", "pos", "meaning"]].copy()
    for c in df.columns:
        df[c] = df[c].astype(str).str.strip()
    # 엑셀 기준 행 번호(헤더 2줄 + 1-based) - 검증 리포트에서 원본 위치를 찾기 위함
    df["xlsx_row"] = df.index + 3
    return df
